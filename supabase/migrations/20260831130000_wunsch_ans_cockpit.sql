-- Änderungswünsche ans CRM (app.epowergmbh.at) weiterreichen.
--
-- Die App schickt, das CRM sammelt nur ein — dadurch braucht das CRM keine
-- Supabase-Schlüssel dieser App. Bilder und Sprachnachrichten bleiben im
-- privaten Bucket; das CRM holt sich bei Bedarf eine signierte URL bei der
-- Edge Function `wunsch-datei`, mit demselben gemeinsamen Geheimnis.
--
-- Solange `cockpit_verbindung` leer ist, tut der Trigger nichts — die
-- Migration lässt sich also gefahrlos vor dem Scharfschalten ausrollen.
...
CREATE TABLE IF NOT EXISTS public.cockpit_verbindung (
  ...
  app_key   text NOT NULL DEFAULT 'monti.pro'
);
