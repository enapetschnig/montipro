...
describe("aggregateMonth — laufender Monat", () => {
  ...
  it("Soll wird auf heute gekappt", () => {
    const mb = aggregateMonth([], 2026, 7, undefined, heute);
    // Mo-Do bis 21.07.: 1,2,6,7,8,9,13,14,15,16,20,21 = 12 Tage
    expect(mb.soll).toBe(120);
  ...
  it("am Monatsersten ist nur dieser eine Tag im Soll", () => {
    const mb = aggregateMonth([], 2026, 7, undefined, new Date(2026, 6, 1));
    expect(mb.soll).toBe(10); // 01.07.2026 ist ein Mittwoch
  });
